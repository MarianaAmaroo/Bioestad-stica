---
title: "Variables aleatorias Dist Cont"
format: html
editor: visual
---

Carga las bibliotecas necesarias para realizar análisis de datos y crear visualizaciones.

-   `tidyverse`: Conjunto de paquetes para manipulación y visualización de datos.
-   `plotly`: Herramienta para crear gráficos interactivos.
...
La distribución uniforme continua describe una variable aleatoria que tiene la misma probabilidad de ocurrir en cualquier intervalo dentro de un rango definido (\[a, b\]).

**Propiedades principales:** - **Media:** ( \frac{a + b}{2} ) - **Varianza:** ( \frac{(b - a)^2}{12} )

Esta distribución es útil para modelar fenómenos donde todos los valores dentro de un intervalo son igualmente probables.
...
tabla_continua <- tibble(x=seq(from=a, to=b, length.out=10), f_x=dunif(x, min=a, max=b))
...
Aquí se genera una visualización que muestra la función de densidad de probabilidad (PDF) de la distribución uniforme.

La PDF describe cómo se distribuyen las probabilidades a lo largo del intervalo (\[a, b\]). La altura constante refleja que todas las sub-regiones del intervalo son igualmente probables.
...
muestra_gamma <- tibble(x = rgamma(N, shape = alfa, rate = lambda), alfa = alfa)

alfa_vector <- c(1.2, 1.8, 2)
for (alfa_j in alfa_vector) {
  tabla_preliminar <- tibble(x = rgamma(N, shape = alfa_j, rate = lambda), alfa = alfa_j)
  muestra_gamma <- bind_rows(muestra_gamma, tabla_preliminar)
}

muestra_gamma$alfa <- factor(muestra_gamma$alfa)

ggplot(muestra_gamma)+
  geom_histogram(aes(x, fill = alfa), color = "black", boundary = 0, binwidth = 0.5)+
  theme_bw()

ggplot(muestra_gamma)+
  geom_density(aes(x, fill = alfa), alpha = 0.1, color = "black")+ 
  theme_bw()
...
```{r}
#| code-fold: true
resumen_gamma <- muestra_gamma |> group_by(alfa) |> summarise(media=mean(x), varianza=var(x))
